...
data/folderHierarchy.json (folder names only, generated by scripts/folder-hierarchy-generator.py)
    ...
    │
    ├─ scripts/fetchSpotifyArtists.ts (artist name → Spotify ID)
    │   └─ data/generated/spotifyArtistManifest.json (2825/3099 matched)
    │
    └─ scripts/fetchSpotifyAlbums.ts (album matching + track fetching + art download)
...
- **Spotify scripts**: Reuse auth, rate limiting (5 concurrent, 200ms delay), retry with backoff, `.env` loading. Pattern in `fetchSpotifyArtists.ts` is the template.
